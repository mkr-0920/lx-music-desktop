<template>
  <material-modal :show="show" :bg-close="bgClose" :teleport="teleport" @close="handleClose">
    <main :class="$style.main">
      ...
      <base-btn :class="$style.btn" @click="handleClick('atmos')">{{ $t('download__atmos') }} - ATMOS</base-btn>
      <base-btn :class="$style.btn" @click="handleClick('master')">{{ $t('download__master') }} - MASTER</base-btn>
    </main>
  </material-modal>
</template>
