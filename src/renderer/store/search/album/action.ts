...
export const search = async(text: string, page: number, sourceId: LX.OnlineSource | 'all'): Promise<ListInfoItem[]> => {
  ...
  if (sourceId == 'all') {
    ...
    for (const source of sources) {
      ...
      // 【修改点 1】：强转 any 绕过 TS，并加上 ?.album?.search 确保安全调用
      const searchPromise = (music as any)[source]?.album?.search?.(text, page, listInfos.all.limit) ??
        Promise.reject(new Error('source not found or album not supported: ' + source))
    ...
    // 【修改点 2】：同样加上强转和多重 ?. 保护
    const searchReq = (music as any)[sourceId]?.album?.search?.(text, page, listInfo.limit)

    return (searchReq ? searchReq.then((data: SearchResult) => {
      if (key != listInfo.key) return []
      return setList(data, page, text)
    }) : Promise.reject(new Error('source not found or album not supported: ' + sourceId))).catch((error: any) => {
